...
module.exports.run = (client, message, args) => {
    ...
    if (!message.member.roles.find("name", "Owner Of  Bruh" , "Mute Hammer")) {
        return message.channel.send(' **Bu Komutu Kullanmak İçin** \*`Owner Of  Bruh*\` **rolune sahip olmalısın** ')
...
let kullanici = message.mentions.members.first() || message.guild.members.get(args[0])
  ...
  if(kullanici.hasPermission("MANAGE_MESSAGES")) return message.channel.send("Ben yetkili birini susturamam.");
...
exports.conf = {
    enabled: true,
    guildOnly: true,
    aliases: ["sescezası", "sesli-mute"],
    permLevel: 0
};

exports.help = {
    name: 'seslimute',
    description: 'seslide mute',
    usage: "seslimute"
};